...
#在smbus2库中，SMBus和i2c_msg是两个重要的类，
# 它们分别用于表示I2C总线和I2C消息。
...
def setPWMServoAngle(index, angle):
    print('setPWMServoAngle(index, angle)')
...
def setPWMServoPulse(servo_id, pulse = 1500, use_time = 1000):
    print(f'setPWMServoPulse({servo_id}, {pulse} , {use_time})')
    if servo_id< 1 or servo_id > 6:
        raise AttributeError("Invalid Servo ID: %d" %servo_id)
    index = servo_id - 1
...
def getPWMServoAngle(servo_id):
    print('getPWMServoAngle(servo_id)')
    if servo_id < 1 or servo_id > 6:
        raise AttributeError("Invalid Servo ID: %d"%servo_id)
    index = servo_id - 1
    return __servo_pulse[index]
...
def setBusServoID(oldid, newid):
    ...
    :param newid: 新的id
    """
    print('setBusServoID(oldid, newid)')
...
def setBusServoAngleLimit(id, low, high):
    ...
    :return:
    '''
    print('setBusServoAngleLimit(id, low, high)')
...
def getBusServoAngleLimit(id):
    ...
    '''
    print('getBusServoAngleLimit(id)')
    while True:
